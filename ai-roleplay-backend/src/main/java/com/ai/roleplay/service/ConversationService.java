...
public class ConversationService extends ServiceImpl<ConversationMapper, Conversation> {
    ...
    public String chat(Long userId, Long conversationId, String content, Long skillId) {
        ...
        if (character == null) {
            throw new IllegalArgumentException("角色不存在，ID: " + conversation.getCharacterId());
        }
        ...
        if (skillId != null) {
            ...
            if (skill != null) {
                response = aiServiceFactory.chatWithSkill(
                ...
                response = aiServiceFactory.chat(character.getSystemPrompt(), messages);
            }
        } else {
            // 普通对话
            response = aiServiceFactory.chat(character.getSystemPrompt(), messages);
